...
mtype SSD::getMeasure(const Mat &f1, Point p1, const Mat& f2, Point p2,
		int WSize)
{
...
void NCC::calcMean(const Mat& f1, Point p1, const Mat& f2, Point p2,
		int WSize)
{
	int WHalf = floor(WSize / 2);
	int f1i, f1j, f2i, f2j;
...
mtype NCC::getMeasure(const Mat& f1, Point p1, const Mat& f2, Point p2,
		int WSize)
{
	int WHalf = floor(WSize / 2);
	int f1i, f1j, f2i, f2j;
...
mtype NCC::getMeasureT(const Mat& f1, Point p1, const Mat& f2, Point p2,
		int WSize)
{
	int WHalf = floor(WSize / 2);
	int f1i, f1j, f2i, f2j;

	mtype diff1, diff2, sigmaf1 = 0, sigmaf2 = 0;
	mtype sum = 0;

	this->calcMean(f1,p1,f2,p2,WSize);
	...
	for (int wi = -WHalf; wi <= WHalf; ++wi)
	{
		...
		for (int wj = -WHalf; wj <= WHalf; ++wj)
		{
			...
			diff1 = f1P[f1j] - this->f1mean;
			diff2 = f2P[f2j] - this->f2mean;
	...
	return sum / sqrt(sigmaf1 * sigmaf2);
}
